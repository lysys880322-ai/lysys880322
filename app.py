...
def section_header(number: str, title: str) -> None:
    st.markdown(
        f'<div class="step-badge"><span class="num">{number}</span>{title}</div>',
        unsafe_allow_html=True,
    )
...
with st.container(border=True):
    section_header("1", "Google 계정 로그인")
...
with st.container(border=True):
    ...
    if st.session_state.translation_rows:
        ...
        # ── 원본 언어(defaultLanguage) 미설정 영상 확인 ──
